...
export default function Dashboard() {
  ...
  /* 🔥 CENTRAL ACTION LOADER */
  const [action, setAction] = useState(null); // "Updating status..."
  ...
  const filteredWorkflows = workflows
    .map(w => ({
      ...w,
      tasks: filter === "all" ? w.tasks : w.tasks.filter(t => t.status === filter),
    }))
  ...
  return (
    <>
      {/* 🔥 CENTER LOADER */}
      <Backdrop open={!!action} sx={{ zIndex: 2000, background: "rgba(255,255,255,0.6)" }}>
      ...
      <Box sx={{ background: "#f6f8fb", minHeight: "100vh", py: 4 }}>
        <Box sx={{ maxWidth: 1100, mx: "auto", px: 2 }}>
          ...
          {/* 🔥 STAT CARDS */}
          <Box display="grid" gridTemplateColumns="repeat(auto-fit,minmax(220px,1fr))" gap={3} mb={4}>
            {[
